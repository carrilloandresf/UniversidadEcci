...
with open('puma.json', 'r') as f:
    puma_data = json.load(f)

with open('toyota.json', 'r') as f:
    toyota_data = json.load(f)

with open('apple.json', 'r') as f:
    apple_data = json.load(f)

with open('pepsi.json', 'r') as f:
...
class Ui_Dialog(object):
    ...
    def move_servos(self, target_angle1, target_angle2, steps=2, delay=0.01):
        # Validar que 'steps' sea un entero positivo
        if steps <= 0:
            raise ValueError("Steps must be a positive integer")
    ...
    def draw_logo(self, logo_name):
        ...
        if logo_name in logo_points:
            ...
            for i, (x, y) in enumerate(points):
                ...
                # Mostrar la barra de progreso y coordenadas/ángulos en una sola línea
                progress_bar_length = 30
                filled_length = int(progress_bar_length * (i + 1) // total_points)
                bar = '=' * filled_length + '-' * (progress_bar_length - filled_length)
                sys.stdout.write(f"\rProgreso: |{bar}| {progress:.2f}% - Tiempo restante: {remaining_time:.2f} s | coord: ({x:.2f}, {y:.2f}) | Th1: {theta1:.2f}, Th2: {theta2:.2f}")
                sys.stdout.flush()
